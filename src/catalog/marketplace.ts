...
export class Marketplace {
  ...
  private reviewsBySkill: Map<string, string[]> = new Map();

  addListing(skill: Skill, isPublic = false): MarketplaceListing {
  ...
  private updateListingRating(skillId: string): void {
    const listing = this.listings.get(skillId);
    if (!listing) return;

    const skillReviews = (this.reviewsBySkill.get(skillId) || [])
      .map(id => this.ratings.get(id))
      .filter((r): r is SkillRating => r !== undefined);
  ...
  addSkillToBundle(bundleId: string, skillId: string): SkillBundle | null {
    ...
    if (bundle.skillIds.includes(skillId)) return bundle;
  ...
  getTopRated(limit = 10): MarketplaceListing[] {
  ...
  getMostInstalled(limit = 10): MarketplaceListing[] {
    return this.getPublicListings()
  ...
  static fromJSON(json: string): Marketplace {
    ...
    for (const [id, listing] of data.listings) {
      marketplace.listings.set(id, listing as MarketplaceListing);
    }
    for (const [id, rating] of data.ratings) {
      marketplace.ratings.set(id, rating as SkillRating);
    }
    for (const [id, bundle] of data.bundles) {
      marketplace.bundles.set(id, bundle as SkillBundle);
    }
